...
export default function PinnedMessages({
  ...
  return (
    <div className="border-b border-yellow-500/20 bg-yellow-500/10 backdrop-blur-sm">
      {displayMessage.map((msg, index) => (
        <div
          key={msg._id}
          className={`px-3 py-2 flex items-start gap-2 ${
            index > 0 ? 'border-t border-yellow-500/20' : ''
          }`}
        >
          ...
          {canModerate && (
            <button
              ...
              className="p-1 rounded hover:bg-white/10 text-gray-500 hover:text-gray-400 transition-colors"
            >
              <X className="w-3 h-3" />
            </button>
          )}
      ...
      {messages.length > 1 && (
        <button
          ...
          className="w-full px-3 py-1 flex items-center justify-center gap-1 text-[10px] text-gray-500 hover:text-gray-400 transition-colors border-t border-yellow-500/20"
        >
          {expanded ? (
            <>
