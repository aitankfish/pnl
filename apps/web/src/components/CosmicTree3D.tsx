...
function RootGlow({ startTime }: { startTime: number }) {
  const meshRef = useRef<THREE.Mesh>(null);
  const matRef = useRef<THREE.MeshBasicMaterial>(null);
  useFrame(({ camera, clock }) => {
    ...
    if (matRef.current) {
      ...
      matRef.current.opacity = eased * 0.95;
    }
  });
  return (
    <mesh ref={meshRef} position={[0, -5.0, -0.3]} renderOrder={-1}>
      <circleGeometry args={[0.58, 96]} />
      <meshBasicMaterial ref={matRef} color="#e89628" transparent opacity={0} toneMapped={false} />
    </mesh>
  );
}
...
function Scene({ markets, startTime, onHover }: { markets: LiveMarket[]; startTime: number; onHover: (m: LiveMarket | null, px?: number, py?: number) => void }) {
  ...
  useFrame(({ clock }) => {
    if (!groupRef.current) return;
    groupRef.current.rotation.z = Math.sin(clock.elapsedTime * 0.15) * 0.02;
    groupRef.current.rotation.y = Math.sin(clock.elapsedTime * 0.1) * 0.015;
  });
  return (
    <group ref={groupRef} position={[0, 0.6635, 0]} scale={0.48}>
      <RootGlow startTime={startTime} />
...
export default function CosmicTree3D({ markets }: { markets: LiveMarket[] }) {
