...
// NOTE: parseClaimAmount is imported from the web app's transaction-parser.
// For the shared package, consumers must provide this or it will be a peer dependency.
// Import path in web app: @/lib/solana/transaction-parser
// This is kept as a dynamic import pattern so it can be overridden per-platform.
type ParseClaimAmountFn = (connection: any, signature: string) => Promise<{ amount: number; type: 'sol' | 'token' }>;

let _parseClaimAmount: ParseClaimAmountFn | null = null;

export function setParseClaimAmount(fn: ParseClaimAmountFn) {
  _parseClaimAmount = fn;
}
  ...
  const claim = async (params: {
    ...
    try {
      ...
      try {
        ...
        if (_parseClaimAmount) {
          console.log('Parsing actual claim amount from transaction...');
          const parseResult = await _parseClaimAmount(connection, signature);
