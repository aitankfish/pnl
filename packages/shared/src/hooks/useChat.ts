...
// IChatMessage type definition (previously from @/lib/mongodb)
...
export function useChat({ marketAddress, walletAddress, autoJoin = true, getAccessToken }: UseChatOptions) {
  ...
  const fetchMessages = useCallback(async (before?: string) => {
    try {
      ...
      const response = await fetch(apiUrl(`/api/chat/${marketAddress}?${params.toString()}`));
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      const response = await fetch(apiUrl(`/api/chat/${marketAddress}`), {
        method: 'POST',
        headers,
        body: JSON.stringify({
  ...
  const addReaction = useCallback(async (messageId: string, emoji: string): Promise<boolean> => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/chat/${marketAddress}/react`), {
        method: 'POST',
  ...
  const deleteMessage = useCallback(async (messageId: string): Promise<boolean> => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/chat/${marketAddress}/${messageId}`), {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  const togglePin = useCallback(async (messageId: string, pinned: boolean): Promise<boolean> => {
    ...
    try {
      const response = await fetch(apiUrl(`/api/chat/${marketAddress}/pin`), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
